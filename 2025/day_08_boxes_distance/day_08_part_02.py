...
# Find first connection which causes all of the junction boxes to form a single circuit
...
all_box_numbers = set(range(len(box_coordinates)))
sorted_boxes_by_dist = sorted(all_box_distance)
...
for box in sorted_boxes_by_dist:
    ...
    if len(circuits) == 1 and set(circuits[0]) == all_box_numbers:
        print('All boxes are now connected!')
        ...
        print('Multiplying X-coordinates of two last boxes that create one circuit from all the boxes:')
        print(box_coordinates[box_01_id][0] * box_coordinates[box_02_id][0])
        break
